...
# Module-level converter, created once and reused across all calls.
_converter = None
...
def _get_converter():
    ...
    Docling is pinned to the CPU accelerator. This project is CPU-only, and CPU
    is also a correctness requirement on Apple Silicon: the layout model
    (rt_detr_v2) builds float64 position embeddings, and the MPS backend raises
    ``Cannot convert a MPS Tensor to float64``, so auto-selecting MPS fails every
    PDF. CPU avoids that entirely.
    """
    global _converter  # noqa: PLW0603
    if _converter is None:
        # All lazy, which keeps module import cheap and Docling-free for unit tests.
        try:
        ...
        # CPU with bounded threads. The CPU-only requirement plus the MPS-float64
        ...
        # on, since tables are where naive RAG loses information, but it runs in
        # FAST mode, which is markedly cheaper than the default ACCURATE TableFormer.
        pipeline_options.do_ocr = False
        pipeline_options.do_table_structure = True
        pipeline_options.table_structure_options.mode = TableFormerMode.FAST
        # Don't retain rendered bitmaps. Page/picture images balloon peak memory
        # and we never embed images; text and table structure are all we chunk.
