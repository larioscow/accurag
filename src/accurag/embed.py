...
def make_client(api_key: str | None = None) -> Any:
    ...
    # resolution (the ambient OPENAI_API_KEY env var). So an unset per-instance
    # key falls back to the environment rather than failing — matches SDK norms.
...
class _LocalEmbeddings:
    ...
    def create(self, model: str, input: list[str]) -> Any:  # noqa: A002 - match OpenAI sig
        ...
        # would mean reloading a heavy model per call). The owning
        # LocalEmbeddingClient exposes the real model via ``.model_name``.
...
class LocalEmbeddingClient:
    ...
    Drop-in replacement for the OpenAI client in :func:`embed_texts` and for
    ``RagPipeline(embed_client=...)``, letting ingestion + retrieval run with
    **no API key and no network** (after the model is cached on first use).
